...
const Products = () => {
  ...
  const getProducts = async () => {
    ...
    products.forEach((product: { id: string }) => {
      if (!product.id || product.id === "0") {
        console.warn(
          `Producto con id inválido encontrado: ${JSON.stringify(product)}`
        );
      }
    });
  ...
  const handleDeleteProduct = async () => {
    if (productToEdit) {
      setProducts(prevProducts =>
        prevProducts.filter(product => product.id !== productToEdit.id)
      );
      setProductsPoint(prevProducts =>
        prevProducts.filter(product => product.id !== productToEdit.id)
      );
  ...
  const currentProducts = productsPoint.slice(
    currentIndex,
  ...
  return (
    <>
      ...
      <div className="flex flex-col pl-16 pt-12 px-10 h-[100%] max-w-[clamp(1000px,77.2vw,1200px)]">
        ...
        <p className="text-[23px] font-bold text-expresscash-textos mb-4">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex gap-7">
            {currentProducts.map((product, key) => (
              <div
                className="max-w-[301px] h-[207px] flex border-[1px] rounded-[13px] border-expresscash-gray"
                key={key}
              >
                <div className="flex flex-col justify-between pt-5 pb-3 pl-4">
                  <h4 className="w-[141px] text-[22px] font-book leading-[28px] text-expresscash-textos mb-1">
                    {truncateName(product.name)}
                  </h4>
                  <h6 className="w-[141px] text-[18px] font-book leading-[22px] text-expresscash-textos mb-1">
                    {product.category}
                  </h6>
                  <h6
                    className="w-[141px] text-[16px] font-book leading-[20px] text-expresscash-textos mb-3"
                  ...
                  <div className="flex justify-between items-center">
                    <p className="text-expresscash-red text-[18px] font-bold leading-[22px]">
          ...
          <div
            ...
            className="w-[301px] h-[207px] flex border-[1px] rounded-[13px] border-expresscash-gray items-center justify-center cursor-pointer"
          >
            <IconMas />
          </div>
        ...
        <h4 className="text-[23px] font-bold text-expresscash-textos pt-10 mb-5">
          Argencompras
        </h4>
        <input
          className="w-full rounded-[13px] border-[1px] border-expresscash-textos border-solid px-6 py-3"
          type="search"
        ...
        <div className="flex justify-between items-end mt-10">
          <p
            ...
            className={`text-[1rem] font-book cursor-pointer transition-all hover:font-medium hover:text-expresscash-black ${categoryFilter !== null ? "text-expresscash-textos cursor-pointer" : "text-expresscash-black font-medium"}`}
          >
            Todos
          </p>
          {categories.map((category: ICategory) => (
            <div key={category.id} className="max-w-[959px]">
              <p
                ...
                className={`text-[0.9rem] cursor-pointer font-book transition-all hover:font-medium hover:text-expresscash-black ${
                  categoryFilter?.id !== category.id
                    ? "text-expresscash-textos cursor-pointer"
                    : "text-expresscash-black font-medium"
                }`}
              >
                {category.name.es}
              </p>
